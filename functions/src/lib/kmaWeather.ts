 ...
 *
 * **실측값입니다(2026-08-18, 14시 발표본).** 오늘 + 4일까지 왔고 양 끝은 부분이었습니다 —
 * 오늘은 남은 시각만, +4일은 오전까지만. 문서는 "3~4일"이라 적혀 있었는데 3으로
 * 잡아두면 **데이터가 있는 하루를 버립니다.**
 *
 * 이 숫자는 "기상청을 부를지" 정하는 문턱일 뿐이고, **최종 판단은 받아온 발표본에
 * 그 날짜가 실제로 있는지**입니다(없으면 out_of_range). 그래서 기상청이 범위를
 * 조정해도 엉뚱한 값이 나오지 않습니다.
 */
export const FORECAST_RANGE_DAYS = 4;
